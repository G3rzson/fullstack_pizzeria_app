...
vi.mock("../_actions/deletePastaAction", () => ({
  deletePastaAction: vi.fn(),
}));
...
const mockDeletePastaAction = deletePastaAction as Mock;
const mockToastSuccess = toast.success as Mock;
const mockToastError = toast.error as Mock;

describe("DeletePastaBtn component", () => {
  beforeEach(() => vi.clearAllMocks());

  it("renders 'Tészta törlése' trigger button", () => {
    render(<DeletePastaBtn id="t1" publicId={null} />);
  ...
  it("renders confirmation description text", () => {
    ...
    expect(screen.getByTestId("modal-description")).toHaveTextContent(
      "Biztos törölni szeretnéd a tésztát",
    );
  });

  it("calls deletePastaAction with id and null publicId on confirm", async () => {
    const user = userEvent.setup();
    mockDeletePastaAction.mockResolvedValue({
      success: true,
    ...
    await waitFor(() => {
      expect(mockDeletePastaAction).toHaveBeenCalledWith("t1", null);
    });
  });

  it("calls deletePastaAction with publicId when provided", async () => {
    const user = userEvent.setup();
    mockDeletePastaAction.mockResolvedValue({
      success: true,
      message: BACKEND_RESPONSE_MESSAGES.SUCCESS,
    ...
    render(<DeletePastaBtn id="t1" publicId="cloud-id" />);
    await user.click(screen.getByTestId("modal-trigger"));

    await waitFor(() => {
      expect(mockDeletePastaAction).toHaveBeenCalledWith("t1", "cloud-id");
    });
  });
  ...
  it("shows server error toast when action throws", async () => {
    const user = userEvent.setup();
    mockDeletePastaAction.mockRejectedValue(new Error("network error"));
  ...
  it("disables trigger button while action is loading", async () => {
    const user = userEvent.setup();
    mockDeletePastaAction.mockImplementation(() => new Promise(() => {}));
